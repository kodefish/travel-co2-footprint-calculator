...
public class FeatureExtractor {
    ...
    protected static ArrayList<Double> linspace(double start, double end, int num) {
        if (num == 1)
    ...
    // N = #samples
    protected static DDL get_fft_values(ArrayList<Double> y_values, double T, int N) {
    ...
    protected static double get_percentile(ArrayList<Double> numbers, double percentile) {
        percentile /= 100;
    ...
    protected static ArrayList<Integer> detect_peaks(ArrayList<Double> x, double mph) {
        if (x.size() < 3)
    ...
    protected static int nextPowerOf2(int n) {
        // by Anshika Goyal, only works for 32bit ints
    	n--; 
        n |= n >> 1; 
        n |= n >> 2; 
        n |= n >> 4; 
    ...
    public static ArrayList<Double> extract_features(ArrayList<Double> data, int window_size) {
        ...
        if (newSize > n)
            data.addAll(Collections.nCopies(newSize - n, 0.0));

        double T = window_size / 1000.0 / n;

        DDL vals = get_fft_values(data, T, data.size());
        ...
        return peaks_x_y; // size = no_peaks * 2
    }
    ...
    /*public static void main(String[] args) {
        FeatureExtractor m = new FeatureExtractor();
        m.test();
    }*/
}
